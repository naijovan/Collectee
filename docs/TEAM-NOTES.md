> **Internal team notes.** This was the repo README until the hackathon
> submission README replaced it at the root. Kept because the merge rules,
> ownership map and honest-pitch notes are still the team's working agreement.
>
> ⚠️ The "AI assistant — where the API key goes" section below is **superseded and
> wrong**. It describes a client-side Gemini key (`EXPO_PUBLIC_GEMINI_API_KEY`).
> The app now calls Anthropic through a server-side Vercel proxy and the key is
> `ANTHROPIC_API_KEY`, set in Vercel's environment — never an `EXPO_PUBLIC_*`
> variable, because those are bundled into the shipped JavaScript. See the root
> `README.md` for the current setup.
