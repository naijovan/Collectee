...
import { catalogueService, collectionService, inventoryService, socialService } from '@/services';
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setGate(
        found
          ? roomEligibility(
              owned.filter((entry) => found.itemIds.includes(entry.itemId)),
              (ownedItemId) => socialService.isUnderReview(ownedItemId),
            )
          : null,
      );
      setBusy(false);
